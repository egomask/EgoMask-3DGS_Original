...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    print("Model loaded successfully, start training")
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        feature_map, depth_image,image_color, viewspace_point_tensor, visibility_filter, radii = render_pkg["feature_map"], render_pkg["depth"], render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
        ...
        if not (iteration < opt.semantic_start or iteration > opt.semantic_stop):
            ...
            # Load ground truth depth image and convert it to a tensor
                # Load ground truth depth image and convert it to a tensor
            # depth_image_gt = Image.open(depth_image_name).convert('L')
            # depth_image_gt_tensor = transform(depth_image_gt).unsqueeze(0).to('cuda')  # Add batch dimension and move to CUDA
            # depth_min_gt, depth_max_gt = depth_image_gt_tensor.min(), depth_image_gt_tensor.max()
            # depth_normalized_gt = (depth_image_gt_tensor - depth_min_gt) / (depth_max_gt - depth_min_gt)

            
            # depth_image_pred_resized = F.interpolate(depth_image.unsqueeze(0), size=(depth_image_gt_tensor.shape[2], depth_image_gt_tensor.shape[3]), mode='bilinear', align_corners=True)

            # # Normalize the resized predicted depth image
            ...
            loss_color = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - L_sim)

            # Normalize depth for loss computation if necessary

            # loss_depth= l1_loss(depth_normalized_pred, depth_image_gt_tensor)

            if iteration >= opt.warm_up and iteration <= opt.semantic_start:
                # loss =   0.2*opt.loss_reduce * loss_feature + loss_color +0.8* opt.loss_reduce * loss_depth
                loss =   0.2*opt.loss_reduce * loss_feature + loss_color
            else:
                # loss =  0.8 * opt.loss_reduce * loss_feature + loss_color + 0.2* opt.loss_reduce * loss_depth
                loss =  0.8 * opt.loss_reduce * loss_feature + loss_color
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int,
                        default=[100, 3000, 5000, 6000, 7_000] + list(range(10000, 40001, 1000)))
