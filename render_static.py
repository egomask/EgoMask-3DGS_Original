...
from gaussian_renderer import render, render_static, render_dynamic
import torchvision
...
def render_set(model_path, load2gpu_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        time_input = fid.unsqueeze(0).expand(xyz.shape[0], -1)
        d_xyz, d_rotation, d_scaling = deform.step(xyz.detach(), time_input)
        results = render_static(view, gaussians, pipeline, background, d_xyz, d_rotation, d_scaling, is_6dof)
        rendering = results["render"]
        ...
        fix_view_results =  render_static(fix_view, gaussians, pipeline, background, d_xyz, d_rotation, d_scaling, is_6dof)
...
def render_sets(dataset: ModelParams, opt: OptimizationParams, iteration: int, pipeline: PipelineParams, frame : int, points : list, thetas : list, prompt : str, novel_views : int):
    ...
    with torch.no_grad():
        ...
        if dataset.fundation_model == "DINOv2":
            ...
            render_set(dataset.model_path, dataset.load2gpu_on_the_fly, dataset.is_6dof, "train_static", scene.loaded_iter,
