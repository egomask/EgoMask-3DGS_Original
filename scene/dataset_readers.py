...
def readColmapSceneInfo(path, images, eval=False, llffhold=20):
    try:
    ...
    # Saving train and test datasets to JSON
    train_json_path = os.path.join(path, "train_cameras.json")
    test_json_path = os.path.join(path, "test_cameras.json")

    json_train_cams = [camera_to_JSON(cam.image_name, cam) for cam in train_cam_infos]
    with open(train_json_path, 'w') as file:
        json.dump(json_train_cams, file)

    json_test_cams = [camera_to_JSON(cam.image_name, cam) for cam in test_cam_infos]
    with open(test_json_path, 'w') as file:
        json.dump(json_test_cams, file)

    print(f"Saved {len(train_cam_infos)} cameras for training to {train_json_path}.")
    print(f"Saved {len(test_cam_infos)} cameras for testing to {test_json_path}.")
